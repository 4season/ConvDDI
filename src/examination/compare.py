...
def batch_check_deep_folders(base_folder_path, csv_file_path, csv_column_name="성분명1"):
    ...
    # 1. Recursively collect all .json files under the specified base directory
    if not os.path.isdir(base_folder_path):
        print(f" Base folder not found: {base_folder_path}")
        return

    # Using recursive=True and '**/*.json' allows scanning the current directory and all subdirectories.
    ...
    for file_path in json_files:
        try:
            ...
            # Extract data from the path: images -> dl_material
            images = json_data.get("images", [])
            for img in images:
                material = img.get("dl_material")
                if material:
                    # Convert to string and strip whitespace to prevent matching errors
                    json_ingredients.add(str(material).strip())
            parsed_count += 1
        except Exception as e:
            # Print the relative path of the faulty file for easy debugging
    ...
    # 2. Load the reference file (supports both .xlsx and .csv)
    ...
    # 3. Validate column existence and extract data
    ...
    # 4. Cross-analysis using set operations
    ...
    if only_in_csv:
        # Display up to 10 samples if the list is too long
        csv_show = list(only_in_csv)[:10] if len(only_in_csv) > 10 else list(only_in_csv)
        print(f"   CSV Sample Details (Top 10): {csv_show} ...")
    print("=" * 75)

    # 6. Export missing ingredients to a CSV file if any discrepancies exist
    if only_in_json:
...
if __name__ == "__main__":
    # 1. Path to the root directory containing multiple category subfolders
    # Example structure:
    # ./all_json_data/
    #    ├── Category_A/
    #    │     └── 1.json
    ...
    # 2. Path to your reference CSV file
    csv_path = PROJECT_ROOT / "data" / "contraindicated_drugs.xlsx"

    # 3. Target column name in the file that contains the ingredient names
    CSV_COL = "material_1"

    # Execute the deep cross-check
